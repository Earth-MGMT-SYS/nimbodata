...
extend(EntityMenu, function () {
    ...
    return {
        ...
        edit_entity_info: function (info) {
            var modalnode = controls.modal()
            if (info.dobj && info.dobj.info) {
                var content = modalnode.append('div')
                var middle = content.append('div')
                var inner = middle.append('div')
                    .html(info.dobj.info)
            ...
            var thiswidget = this
            middle.classed('n_editable',true)
            middle.append('div')
            ...
            content
                .on('click',function () {
                    ...
                    if (content.classed('n_empty')) {
                        content.classed('n_empty',false)
                        inner.text('')
                    }
                    
                    middle.classed('n_edit_mode',true)
                    
                    var editor = inner
                        .classed('n_empty',false)
                        .on('click',null)
                        .attr('contenteditable',true)
                    
                    var cked = CKEDITOR.inline(editor.node())
                    
                    middle.select('div.fa')                        
                        ...
                        .on('click',function () {
                            middle.select('div.fa')
                                ...
                                .classed('fa-pencil',true)
                                .on('click',function () {
                                    thiswidget.edit_entity_info()
                                })
                            
                            middle
                                .classed('n_edit_mode',false)
                            editor.attr('contenteditable',false)
                            var obj = Nimbodata.Entity(objid)
                                .modify({dobj:{info:editor.html()}},function () {})
                            cked.destroy()
                        })
                })
        },
        ...
        update: function (e, dataset) {
            ...
            var on = {
                click: function () {
                    ...
                    if (action == 'info') {
                        thiswidget.edit_entity_info(info)
                        return
                    }
                    if (action == 'drop') {
                        var really = confirm("Really drop " + info.name + " ? ")
                        if (really) {
                            Model.respond(self,'clear',info)
