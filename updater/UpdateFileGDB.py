...
class UpdateFileGDB(object):
    """A database drawn from an ESRI File Geodatabase via OGR."""
...
class UpdateFileGDBTable(object):
    """A table from a GDB zip file."""
    ...
    def __init__(self,fname,fpath):
        if os.path.splitext(fpath)[1] == '.zip':
            ...
            fpath = os.path.splitext(fpath)[0] + '.gdb'
    ...
    def columns(self):
        ...
        p = subprocess.check_output(['ogr2ogr','-f "PostgreSQL"',self.fpath])
...
class UpdateFileGDBURL(UpdateShapefileTable):
